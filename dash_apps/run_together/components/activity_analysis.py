...
def get_activity_analysis(extended_activity: ExtendedActivity):
    ...
    for pace_zone, zone_threshold in zones_threshold.items():
        ...
        if pace_zone in list(zone_pace_heart_rate_kpi.keys()):
            ...
            if zone_pace_heart_rate_kpi[pace_zone]['total_distance'] > zone_threshold['threshold']:
                zones_to_display.append({
                    'distance': zone_pace_heart_rate_kpi[pace_zone]['total_distance'],
                    'pace_zone': pace_zone
                })
                i = i + 1
    ...
    sorted_zones_to_display = sorted(zones_to_display, key=lambda d: d['distance'], reverse=True)
    ...
    logging.info(f"Longest Pace zone for the activity: {longest_pace_zone} "
    ...
    for heart_rate_zone in list(zones_threshold.keys()):
        ...
        if heart_rate_zone in list(zone_pace_heart_rate_kpi[longest_pace_zone['pace_zone']].keys()):
            distance_heart_rate_in_range = distance_heart_rate_in_range + zone_pace_heart_rate_kpi[
    ...
    score_text = ""
    if score > 50:
        score_text = f"🟢 Congrats, {round(score, 2)}% of the distance run in " \
                              f"{sorted_zones_to_display[0]['pace_zone']} Pace is equal or bellow the associated heart-rate. "
    if score <= 50:
        score_text = f"🔴 Be careful, {round(score, 2)}% of the distance run in " \
