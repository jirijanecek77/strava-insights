...
def test_compare_periods_returns_numeric_deltas() -> None:
    summaries = aggregate_period_summaries(
        [
            ActivityAggregateInput("Run", date(2026, 3, 1), Decimal("10000"), 2700, Decimal("100")),
            ActivityAggregateInput("Run", date(2026, 2, 1), Decimal("8000"), 2400, Decimal("50")),
        ],
        period_type="month",
    )
...
def test_summarize_window_builds_rolling_period_metrics() -> None:
    summary = summarize_window(
        [
            ActivityAggregateInput("Run", date(2026, 3, 1), Decimal("10000"), 2700, Decimal("100")),
            ActivityAggregateInput("Run", date(2026, 3, 2), Decimal("5000"), 1500, Decimal("50")),
        ],
        sport_type="Run",
