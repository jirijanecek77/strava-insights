...
def upgrade() -> None:
    op.add_column(
        "user_profiles", sa.Column("aet_heart_rate_bpm", sa.Integer(), nullable=True)
    )
    op.add_column(
        "user_profiles", sa.Column("ant_heart_rate_bpm", sa.Integer(), nullable=True)
    )
    op.add_column(
        "user_profiles",
        sa.Column(
            "aet_pace_min_per_km", sa.Numeric(precision=5, scale=2), nullable=True
        ),
    )
    op.add_column(
        "user_profiles",
        sa.Column(
            "ant_pace_min_per_km", sa.Numeric(precision=5, scale=2), nullable=True
        ),
    )
