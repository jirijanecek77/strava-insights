...
from app.garmin_client import GarminAuthenticationError
from app.logging import reset_log_user_name, set_log_user_name
from app.repositories import GarminCredentialRepository, SyncJobRepository, UserRepository
...
def _handle_authentication_required(session, sync_jobs, *, sync_job_id: int, user_id: int) -> None:
    sync_job = sync_jobs.get(sync_job_id, user_id)
    if sync_job is not None:
        sync_jobs.require_authentication(sync_job)
    GarminCredentialRepository(session).mark_reauthentication_required(user_id)
    session.commit()
...
def run_full_import(*, sync_job_id: int, user_id: int) -> None:
    ...
    try:
        ...
        session.rollback()
        _handle_authentication_required(session, sync_jobs, sync_job_id=sync_job_id, user_id=user_id)
        logger.warning("Full import requires Garmin reauthentication.",
...
def run_incremental_sync(*, sync_job_id: int, user_id: int) -> None:
    ...
    try:
        ...
        session.rollback()
        _handle_authentication_required(session, sync_jobs, sync_job_id=sync_job_id, user_id=user_id)
        logger.warning("Incremental sync requires Garmin reauthentication.",
                       extra={"sync_job.id": sync_job_id, "user.id": user_id})
    except Exception as exc:
        session.rollback()
        sync_job = sync_jobs.get(sync_job_id, user_id)
